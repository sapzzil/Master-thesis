...
Contact: {abhidas, weihaokong, senrajat, yichenzhou}@google.com
...
(Footnotes: Google Trends https://trends.google.com; Wiki Pageview statistics https://wikimedia.org/api/rest_v1/)
...
In other words, the inputs y_{1:L} are broken down into patches of size input_patch_len (p). The j-th patch can be denoted as ỹ_j = y_{p(j-1)+1:pj}. Similarly the mask can also be patched as m̃_j = m_{p(j-1)+1:pj}. Then the j-th input token to the subsequent transformer layers can be denoted as,

t_j = InputResidualBlock(ỹ_j ⊙ (1 − m̃_j)) + PE_j  (2)

where PE_j denotes the j-th positional encoding as defined in the original transformer paper [VSP+17]. There will be N = ⌊L/p⌋ such input tokens.
...
for all j ∈ [N]. ṁ_j is the masking indicator for the j-th token defined as min{m_{p(j-1)+1:pj}} i.e if a patch has any non-masked time-point the corresponding token marked as not being masked. All patches that are masked out completely are not attended to by the causal self attention.
...
**Monash [GBW+21].** Monash archive is a collection of 30 datasets of different training and prediction lengths that covers granularities ranging from minutes to years and domains including finance, demand forecasting, weather and traffic. The archive reports four official metrics for several statistical baselines such as Exponential Smoothing(ETS) and ARIMA, as well as supervised ML baselines like CatBoost [PGV+18], DeepAR [SFGJ20] and WaveNet [ODZ+16]. Following llmtime [GFQW23] we start from the Monash Huggingface repository and filter out the datasets that contain missing values. This leaves us with 18 datasets which we specify in Appendix A.5.2.
...
In Monash benchmarks [GBW+21] ε=0.1 was used. This metric is used in order to avoid undefined values in other normalized metrics like MAPE. In multivariate datasets the metrics are calculated for each time-series and then we take the mean or the median. In this paper we only use the mean versions.
...
**Table 2: MAE of different methods on ETT datasets (10% of training set used for training/finetuning; baseline numbers from Table 13 in [ZZP+21]).**

| Dataset | Horizon | TimesFM(FT) | GPT4TS(FT) | DLinear | PatchTST | TimesNet | FEDFormer | Autoformer |
|---|---|---|---|---|---|---|---|---|
...
Figure 4: We report average performance in three groups of datasets. In all figures, the lower the metric the better and the error bars represent one standard error. Note that among the baselines only TimesFM and llmtime are zero-shot. In (a) we report results on the Monash datasets using Arithmetic Mean (AM) of the MAE's scaled by the MAE of a naive baseline; TimesFM is within significance of the top model N-BEATS. In (b), average scaled MAE on Darts; TimesFM is within significance of the top method ARIMA. In (c), average MAE for 96/192 horizon on 4 ETT datasets; TimesFM and PatchTST are the best performing models.
...
**Table 3: MAE for Darts datasets (including naive baseline that predicts the last values in the context repeatedly).**
...
**Table 4: Mean MAE results for TimesFM and Monash baselines (including naive baseline).**
...
**Table 5: MAE for ETT datasets for prediction horizons 96 and 192 (last test window of original test split).**
...
(Footnote: https://github.com/ngruver/llmtime/blob/main/experiments/run_monash.py)

### A.7 Date Features

As we mentioned earlier, since we are building a single pre-trained model, we cannot have dataset specific dynamic or static covariates during training time. However, the datetime column is ubiquitous in all time-series data, so we can technically have date derived features like day of the week, month of the year etc processed into a vector at each time-point t, denoted by x_t ∈ R^r.
