...
| **Profit Mirage: Revisiting Information Leakage in LLM-based Financial Agents** | arXiv 2510.07920 | **완전 확인** (공식 원본 PDF 보유) | "most systems exhibit a 'profit mirage': dazzling back-tested returns evaporate once the model's knowledge window ends, because of the inherent information leakage in LLMs." 4개 차원 정보 누출 정량화, 누출 저항 벤치마크 **FinLakeBench** 및 반사실적 섭동 프레임워크 **FactFin** 제안. | **사전학습 코퍼스 시계열 누출(Look-ahead Leakage)의 직접 증거.** TSFM/LLM이 금융에서 겉보기 성능이 좋은 것처럼 보이지만 실제로는 과거 데이터를 암기하여 발생한 착시 현상임을 실증. 우리 연구의 리스크 3번(Look-ahead 누출 리스크) 및 사전학습 코퍼스 내 금융 데이터 부재/과소대표 논의를 완벽히 뒷받침. |
