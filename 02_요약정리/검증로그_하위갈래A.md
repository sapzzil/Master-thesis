...
> 검증 방식: `01_자료원문/` 로컬 공식 원본 PDF 전수 대조 (Moirai 25p, Chronos 43p 등)
> **원칙**: 원문에서 직접 읽은 문장만 근거로 인정. 추론은 "추론"으로 명시.
>
> **★ 2026-09-23 2차 갱신 완료 사항**:
> 1. Moirai 1.0 공식 완본 PDF(25p) 대조로 Table 2 LOTSA 도메인별 통계 중 Econ/Fin 0.10%(24,919,596 obs, 23개 데이터셋) 실측치 100% 확정.
> 2. Moirai 부록 C.1 CRPS 적분 정의식 및 $K=9$ 분위수 wQL 핀볼 손실 근사식 수식 확인 완료.
> 3. Sundial TimeBench 1.03조 포인트 수치 확인 완료.
> 4. 과거 1차 검증의 모든 "확인 불가/확인 대기" 항목이 공식 PDF 근거로 전수 해소·종결됨 (상세: `02_요약정리/검증로그_공식PDF_2차전수대조.md` 참조).
